...
modelo_ldaCETotal_MA <- lda(formula = MBI_CE_Cat ~ Neur_NEOFFI+Amab_NEOFFI+Extrav_NEOFFI+Resp_NEOFFI+Apert_NEOFFI+Edad+Sexo+Antigüedad_profesión+Antigüedad_puesto+Turno_Cat+Guardias+Puesto_trabajo+Estado_Civil_Cat+Gestíon+Tiempo_gestión+Ocupó_gestión+Tiempo_ocupó_gestión,data = MBI_CE_MA)
...
modelo_ldaDTotal_MA <- lda(formula = MBI_D_Cat ~ Neur_NEOFFI+Amab_NEOFFI+Extrav_NEOFFI+Resp_NEOFFI+Apert_NEOFFI+Edad+Sexo+Antigüedad_profesión+Antigüedad_puesto+Turno_Cat+Guardias+Puesto_trabajo+Estado_Civil_Cat+Gestíon+Tiempo_gestión+Ocupó_gestión+Tiempo_ocupó_gestión,data = MBI_D_MA)
...
modelo_ldaRPTotal_BM <- lda(formula = MBI_RP_Cat ~ Neur_NEOFFI+Amab_NEOFFI+Extrav_NEOFFI+Resp_NEOFFI+Apert_NEOFFI+Edad+Sexo+Antigüedad_profesión+Antigüedad_puesto+Turno_Cat+Guardias+Puesto_trabajo+Estado_Civil_Cat+Gestíon+Tiempo_gestión+Ocupó_gestión+Tiempo_ocupó_gestión,data = MBI_RP_BM)
...
El error se encuentra entre el 34% y el 42%. De esta forma, se tiene que este modelo se equivoca un número mayor de veces que el obtenido al considerar únicamente las variables de personalidad. 


(Creo que el texto que sigue a continuación de esto podríamos quitarlo de este archivo y solo dejarlo en el final report.)
...
En primer lugar, llevamos a cabo la obtención de los coeficientes del modelo, para cada uno de los síntomas del Burnout, tomando como variables explicativas independientes (pues en las gráficas bivariadas no observamos relación dos a dos.)
...
print(coef_ordenados)
```
Error del modelo para el Cansancio Emocional.

Una vez que disponemos del modelo se procede a comprobar su funcionamiento. 
...
```
Ahora vamos a calcular el p-valor de las variables de personalidad. Cuando el p-valor es menor de 0.05, se rechaza la hipótesis nula, entonces $\beta_i$ es significativo en el modelo. 

p-valores de Cansancio Emocional
...
```
En este caso, las variables significativas son Neuroticismo y Extraversión.

p-valor de la Despersonalización
...
```
En este caso, las variables significativas son el Neuroticismo y la Responsabilidad.

p-valor de Realización Personal.
...
En este caso, la única variable significativa es Neuroticismo. 
